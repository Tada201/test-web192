...
export function ProgressTracker({ title, steps, onStepToggle }: ProgressTrackerProps) {
  const [stepStates, setStepStates] = useState<Record<string, boolean>>(() => {
    const saved = localStorage.getItem(`progress-${title.toLowerCase().replace(/\s+/g, '-')}`);
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <h4 className={`font-medium ${isCompleted ? 'text-green-200' : 'text-doc-text'}`}>
                  {step.title}
                </h4>
                <p className={`text-sm mt-1 ${isCompleted ? 'text-green-300' : 'text-doc-text-muted'}`}>
                  {step.description}
                </p>
              </div>
        ...
        <div className="mt-6 p-4 bg-green-900/20 border border-green-700/30 rounded-lg">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-green-300 text-sm">You've completed all steps in this section.</p>
            </div>
          </div>
        </div>
      )}
